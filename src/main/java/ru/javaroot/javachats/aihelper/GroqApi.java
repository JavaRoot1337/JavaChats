...
public final class GroqApi implements AutoCloseable {
    ...
    public GroqApi(JavaChat plugin, String key, String endpoint, String model, long timeoutSeconds) {
        ...
        this.key = key;
        this.model = model;
        this.http = new HttpAiClient(plugin, key, endpoint, timeoutSeconds, "groq");
    }
    ...
    private AiResult parse(String response, String message, Map<String, AiRules.RuleInfo> rules) {
        try {
            ...
            plugin.getLogs().warning("groq-parse", ru.javaroot.javachats.utils.LogVars.of(
                    "error", String.valueOf(ex.getMessage())));
            throw new IllegalArgumentException("invalid Groq response", ex);
        }
    }
