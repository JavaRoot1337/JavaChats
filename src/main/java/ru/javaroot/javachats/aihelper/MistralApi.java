...
public class MistralApi {
    ...
    public MistralApi(JavaChat plugin, String key) {
        this.plugin = plugin;
        this.key = key;
        ...
        return http.sendAsync(req, HttpResponse.BodyHandlers.ofString()).thenApply(res -> {
            if (res.statusCode() >= 200 && res.statusCode() < 300) {
                try {
                    ...
                    plugin.getLogger().warning("Error parsing Mistral JSON: " + e.getMessage());
                }
            } else {
                plugin.getLogger().warning("Mistral API Error (" + res.statusCode() + "): " + res.body());
            }
            return null;
        });
    }
